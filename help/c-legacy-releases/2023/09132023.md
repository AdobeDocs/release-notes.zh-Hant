...
* **從資料來源大量匯入資產** – 管理員現在可以將資料來源的[多個資產匯入](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/assets-view/bulk-import-assets-view.html?lang=zh-Hant) AEM Assets。 管理員不再需要將個別資產或資料夾上傳至 AEM Assets。 支援大量匯入的資料來源包括 Azure、AWS、Google 雲端以及 Dropbox。
...
* Experience Manager Assets現在包含`damAssetLucene`索引的新版本9。`damAssetLucene-9` 將Oak查詢Facet計數的行為變更為[，不再評估基礎搜尋索引傳回之Facet計數](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/manage/search-assets.html?lang=zh-Hant)的存取控制，這會加快搜尋回應時間。
...
* [[!UICONTROL 最適化表單]中以核心元件為主的表單片段](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-core-components/create-an-adaptive-form-on-forms-cs/adaptive-form-fragments-core-components.html?lang=zh-Hant) – 告別重複資料、最佳化您的數位庫存，並隨著使用[!UICONTROL 表單片段]提升表單建立體驗來提升共同作業。 這些可重複使用的元件可順暢地整合為多種表單，簡化建立一致且具備專業外觀的表單。[!UICONTROL 表單片段]透過&#x200B;_變更一次並反映所有位置_&#x200B;功能，確保可重複使用、標準化和品牌一致性。 在一處進行的更新會自動傳播到使用這些片段的所有表單，因此可體驗更高的可維護性和效率。
* [增強 Acrobat Sign Workflow 步驟](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/create-form-centric-workflows/aem-forms-workflow-step-reference.html?lang=zh-Hant#sign-document-step-sign-document-step) – [!DNL Acrobat Sign] Workflow 步驟已強化，以納入以下內容：
   ...
   * **Acrobat Sign 文件的稽核軌跡** – 使用稽核軌跡功能獲取 Adobe Sign 文件的生命週期的詳細洞察。 有了稽核軌跡，您現在就可以保留與文件相關之所有動作和互動的全面記錄了。 此記錄包括曾檢視、編輯或簽署文件之人員的詳細資料，以及每個事件的時間戳記。 此記錄增強功能對於維持合規性、解決爭議和確保數位協議的完整性至關重要。
...
* 記錄中不再缺少 `contentSetName` 值，現在啟動[內容複製](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developer-tools/content-copy.html?lang=zh-Hant)作業時，在輸入中便會提供該值。
* 在特定的少數情況下，從同一個管道啟動執行兩次，以致於造成「停滯」狀態的情況不再可能發生。
...
* [Experience Manager as a Cloud Service 2023.8.0 - 版本更新](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager-blogs/aem-as-a-cloud-service-2023-8-0-release-update/ba-p/616365?profile.language=zh-Hant)。
* [AEM Community Lens 第 7 版，2023 年 8 月](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager-blogs/adobe-experience-manager-community-lens-7th-edition-august-2023/ba-p/614084?profile.language=zh-Hant)。
* [AEM使用者 — 分享您對Gartner Peer Insights的意見！](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/adobe-experience-manager-users-share-your-feedback-on-gartner/td-p/613702?profile.language=zh-Hant)
