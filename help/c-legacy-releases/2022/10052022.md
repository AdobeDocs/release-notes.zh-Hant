...
| 功能 | 說明 |
| ------- | ----------- |
| [!UICONTROL 產品組合]，[!UICONTROL 行銷活動] | (選擇加入 Beta 版功能) 新的「[!UICONTROL 網路支出條件約束]」選項可讓您為多網路產品組合中的特定廣告網路定義預算分配限制。 該選項適用於具有每日、每周和每月支出策略的產品組合，且僅當「[!UICONTROL 啟用行銷活動最大支出目標 %]」選項未啟用時。 選取「[!UICONTROL 自動調整行銷活動預算限制]」選項時，條件約束會在為多個和最低行銷活動預算設定設定的值範圍內作用。<br><br>基本報告中的新網路條件約束報告會顯示所有指定的最小和最大搜尋引擎預算以及您的產品組合的實際支出（成本）。<br><br>若要參與測試版計劃，請連絡您的客戶團隊。 |
