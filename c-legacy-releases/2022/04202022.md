...
* **Experience Manager Assets as a Cloud Service**
   ...
   * 現在可以使用 Experience Manager Assets 使用者介面以便：
      ...
      * 設定向影像[新增數位浮水印](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/manage/watermark-assets.html)。
...
| [!UICONTROL Advertising Insights] | 除了[!UICONTROL 查詢交叉比對]和[!UICONTROL 混合投資組合設定準備]之外的所有深入分析，現在都已完成測試。 |
| [!UICONTROL 行銷活動]<br><br>[!UICONTROL Advertising Insights] | (4 月 11 日；[!DNL Google Ads] 帳戶) Advertising Cloud Search 已將舊版 [!DNL Google AdWords API] 中的所有 API 呼叫變成最新的 [!DNL Google Ads API]。移到新 [!DNL Google Ads API] 以確保現有功能的連續性，並允許存取 [!DNL Google's] 的最新 [!DNL Ads] 功能。<br><br>某些功能尚未更新到新的 API，所以暫時無法使用：<ul><li>位置延伸模組：<ul>[!UICONTROL 延伸模組]檢視中看不到位置延伸模組。</li><li>您無法建立延伸模組。</li><li>對位置進行篩選無法運作。</li></li></ul><li>[!UICONTROL Advertising Insights]：無法使用[!UICONTROL 錯失的曝光比重]和[!UICONTROL 查詢交叉比對 Beta 版]分析。</li></ul>我們預計在 4 月底之前恢復位置延伸模組。一旦我們確定受影響的 [!UICONTROL Advertising Insights] 模組何時也能恢復，我們會傳送包含估計日期的更新資訊。 |
