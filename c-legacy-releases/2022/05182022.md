...
| 功能 | 說明 |
| ------- | ----------- |
| [!UICONTROL 自訂報告] | (5月31日；測試版功能)Advertising Cloud DSP現在可內嵌第一方區段，由客戶資料平台(CDP)內建的已驗證訊號組成。 |
| 最佳化目標 | （5月25日）視訊和原生刊登版位現在可包含在具有自訂目標的套件中 [!UICONTROL 最高ROAS] 和 [!UICONTROL 最低CPA]. |
...
| 與 Adobe Analytics 整合 | （5月7日）在Advertising Cloud傳送至的資料摘要中 [!DNL Analytics]，資料 [!DNL Google Ads] 自2022年5月7日起，動態搜尋廣告的可用範圍僅限於廣告群組層級。 |
