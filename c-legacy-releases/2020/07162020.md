...
| 以人物為基礎跨裝置圖表目標定位 | 自助式 DSP 客戶可以使用裝置圖表 (Adobe Device Co-op 或 LiveRamp)，針對新的行銷活動進行以人物為基礎的目標定位及頻率管理。這確保廣告商能夠在受眾擁有的裝置上觸及目標對象，並限制曝光度。 此外，跨位置大量編輯跨裝置目標定位設定現在提供 Beta 版。 要為您的帳戶啟用 Beta 版，請連絡您的 Adobe 客戶經理。 |
