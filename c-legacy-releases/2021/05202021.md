...
| 2021 年 5 月 | [在資料檢視中設定元件設定](https://experienceleague.adobe.com/docs/customer-journey-analytics-learn/tutorials/configuring-component-settings-in-data-views.html?lang=zh-Hant) | 影片 | 了解設定時，您將要為每個元件（量度和維度）設定的基本元件設定 [!UICONTROL 資料檢視] Customer Journey Analytics。 |
...
* **Adobe Experience Manager as a Cloud Service**
   ...
   * **Experience Manager Forms as a Cloud Service**
      ...
      * **在啟用 Adobe Sign 的最適化表格中使用政府機關身分證件驗證方法**
         ...
         Adobe Sign的政府ID程式採用進階的機器學習演算法，讓全球公司能夠確保對收件者身分進行高品質驗證。 現在，您可以在啟用 Adobe Sign 的最適化表格中使用政府機關身分證件驗證方法。

         政府機關身分證件是優質的身分驗證方法。 會指示收件者 [上傳政府頒發的身份證件（駕照、國家身份證、護照）的影像](https://helpx.adobe.com/tw/sign/using/adobesign-authentication-government-id.html). 然後，它會評估該證件以確保其真實性。
...
| 功能 | 說明 |
| -----------| ---------- |
| 套件設定 | 全新[!UICONTROL 填寫定速策略]選項「[!UICONTROL 稍微領先]」現已推出，而且是新套件的預設值。 此策略可加快傳送速度，使其在飛行期間完成55-65%。 |
...
| 已發佈 | 名稱 | 類型 | 說明 |
| -----------| ---------- | ---------- | ---------- |
| 2021 年 5 月 | [控制您的 PDF 線上體驗並收集分析](https://experienceleague.adobe.com/docs/document-services/tutorials/pdfembed/controlpdfexperience.html?lang=zh-Hant#part1) | 文章 | 了解如何使用Adobe PDF Embed API控制外觀、啟用共同作業，以及收集有關使用者與PDF互動的分析，包括頁面逗留時間和搜尋。 (四個部分。) |
