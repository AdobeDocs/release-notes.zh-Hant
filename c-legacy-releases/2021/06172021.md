...
| [!UICONTROL 廣告深入分析] | 以下見解在測試模式下可用：<ul><li>[!UICONTROL 標準化SIM（組合）Beta]</li><li>[!UICONTROL Portfolio啟動測試版]</li> |
...
|  | 新警報類型， [!UICONTROL Campaign ManagementUI操作]，可用並顯示所有後台作業的最終狀態，包括對包、約束、分類和事件的批量編輯和分配更改。 您自動訂閱UI操作的警報。<br><br>此警報類型最終將替換右工具欄中提供的舊作業狀態通知（舊作業狀態通知）。 |
