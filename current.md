---
title: 中央發行說明
description: 尋找 Adobe  [!DNL Experience Cloud]  產品和服務的最新發行說明和知識庫問題。了解 Experience League 即將舉辦的活動和新文件。探索  [!DNL Experience Cloud]  應用程式的最新教學課程和其他課程。
doc-type: release notes
last-update: January 2026
author: mfrei
mini-toc-levels: 2
exl-id: 091f0168-21b0-4f48-a02b-d70e96b84e27
source-git-commit: 977e1e623f580589aac7cf08bf4bf81fb099610e
workflow-type: tm+mt
source-wordcount: '5047'
ht-degree: 35%

---

# Experience Cloud Central發行說明 — 2026年1月

<!-- badgeReview: label="Internal Review" type="Negative" -->

此頁面能協助您：

* 尋找應用程式特定發行說明的連結
* 找出事件並瞭解Experience League內容
* 檢視每個應用程式的新影片教學課程
* 查看最新的支援文章

## Experience League上的課程

[課程](https://experienceleague.adobe.com/en/courses)可在Experience League上取得。 他們提供結構化、由專家建立的學習課程，協助您更快速地建立技能、驗證專業知識，並在Adobe解決方案之間更自信地推進。

* 免費提供
* 讓複雜主題輕鬆完成的小型模組
* 可讓您隨時暫停和繼續的進度追蹤
* 知識檢查與測驗可加強每個階段的瞭解與驗證學習
* 免費的、可共用的完成證書，在您符合課程要求時授予

每個課程都以可共用的完成證明結束，讓您在內部或在LinkedIn上強調自己的成就。

[了解更多](https://experienceleague.adobe.com/en/courses)

**此頁面中的產品連結**

+++檢視區段連結

* [活動排程](#events)
* [&#x200B; [!DNL Experience Cloud]中的AI功能](#ai)
* [[!DNL Adobe System Status]](#status)
* [[!DNL Adobe Experience Cloud] - 中央介面與管理](#ecloud)
* [[!DNL Adobe Experience Platform]](#platform)
* [[!DNL Adobe Real-Time CDP]](#rtcdp)
* [[!DNL Adobe Analytics]](#analytics) （1月15日）
* [[!DNL Adobe Customer Journey Analytics]](#cja) （1月15日）
* [[!DNL Adobe Streaming Media Analytics]](#sma)
* [[!DNL Adobe Experience Manager]](#aem)
* [[!DNL Adobe LLM Optimizer]](#llm-optimizer)
* [[!DNL Adobe Brand Concierge]](#brand-concierge)
* [[!DNL Adobe Commerce]](#commerce)
* [[!DNL Adobe Target]](#target)
* [[!DNL Adobe Campaign]](#ac)
* [[!DNL Adobe Journey Optimizer]](#journey-opt)
* [[!DNL Adobe Journey Optimizer B2B Edition]](#ajo-b2b)
* [[!DNL Adobe Marketo Engage]](#marketo)
* [[!DNL Adobe Workfront]](#workfront)
* [[!DNL Adobe GenStudio for Performance Marketing]](#genstudio-marketing)
* [[!DNL Adobe Mix Modeler]](#mix-modeler)
* [[!DNL Adobe Advertising]](#advertising)
* [[!DNL Adobe Pass]](#pass)
* [[!DNL Adobe Document Cloud]](#doc-cloud)
* [[!DNL Adobe Creative Cloud for enterprise]](#creative-cloud)
* [客戶資料管理 - 對話](#voices)
* [數位體驗藍圖](#blueprints)
* [Adobe 的認證](https://experienceleague.adobe.com/zh-hant/certification-home)
* [Adobe 產品安全性漏洞](https://helpx.adobe.com/tw/security.html)

+++

## 活動排程 {#events}

尋找並報名參加您感興趣的[活動](https://experienceleague.adobe.com/zh-hant/events)。

+++檢視即將舉行的活動

* **[!DNL Workfront]** | _管理員101_ | 這套活動專為新管理員設計，讓他們瞭解可用資源、系統導覽摘要，以及有關初始設定、錄取和基本報告的最佳實務。. | **1月15日** | [註冊](https://events.teams.microsoft.com/event/638a55a5-93ba-4e79-ab7a-4f4e8186e559@fa7b1b5a-7b34-4387-94ae-d2c178decee1)

* **[!DNL Workfront]** | Adobe Workfront中的&#x200B;_10個通訊秘訣_ | 有效溝通是成功的基石，而清楚溝通、主動傾聽及周到回應能力的培養，則能帶來天翻地覆的變化。 此工作坊將提供10個可操作的技巧(都在Workfront之外)，您可以立即開始使用以降低不必要問題的風險。  | **1月22日** | [註冊](https://events.teams.microsoft.com/event/925fd550-bbf4-49f7-a3ef-3cb149496a2e@fa7b1b5a-7b34-4387-94ae-d2c178decee1)

* **[!DNL Workfront]** | _Workfront Collective_ | 這是Workfront集體活動 — 所有Workfronters的眾包機會！ | **1月26日** | [註冊](https://events.teams.microsoft.com/event/4035d16b-71b4-4b40-ac52-3f44dd61bf52@fa7b1b5a-7b34-4387-94ae-d2c178decee1)

* **[!DNL Adobe Summit]** | _瞭解行銷、創意和AI的未來_ | 探索行銷、創意和AI的後續發展。 | **拉斯維加斯，4月19至22日** | [註冊](https://summit.adobe.com/na/)

檢視 Experience League 上[即將到來的](https://experienceleague.adobe.com/zh-hant/events)以及[隨選](https://experienceleague.adobe.com/zh-hant/docs/events/experience-league-recorded-events/overview)活動的完整清單。

+++

## Experience League的最新AI資源 {#ai}

搜尋為 Experience Cloud 中的生成式 AI (genAI)、AI 助理和 AI 代理發佈的最新學習資源。

+++詳細資料

| 產品 | 資源 | 說明 | 已更新 |
| ------- | ------- | ------- | ------- |
| [!DNL Experience Manager as a Cloud Service] | AEM中的[Agentic AI](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/overview?#agentic-ai-in-aem) | 瞭解AEM CS中的新AI代理。 | 2025年11月21日 |
| [!DNL Experience Cloud] | [Experience Cloud應用程式中的Agentic AI](https://experienceleague.adobe.com/zh-hant/docs/core-services/interface/features/agentic-ai) | 已發佈[AEM代理程式](https://experienceleague.adobe.com/en/docs/core-services/interface/features/agentic-ai#ai-first-experience-cloud-applications)的新說明。 尋找說明並檢查Experience Cloud中AI代理程式的可用性。 | 2025年11月21日 |
| [!DNL Experience Cloud] | [Experience Cloud 應用程式中的生成式 AI](https://experienceleague.adobe.com/zh-hant/docs/core-services/interface/features/generative-ai) | 瞭解Experience Cloud中可用AI支援的功能的位置。 | 2025 年 11 月 |
| AI 文件首頁 | [AI 文件](https://experienceleague.adobe.com/zh-hant/docs/ai) | 若要尋找 Experience Cloud 中 AI 驅動功能 (生成式 AI 和 AI 代理) 相關資訊的連結，請造訪新產品文件登陸頁面。 | 2025 年 10 月 |
| [!DNL AEM CS] | [AEM AI 助理](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-learn/cloud-service/aem-ai-assisstant) | 了解 Experience Manager AI 助理如何簡化工作流程。 | 2025 年 10 月 |
| [!DNL Experience Platform] | [Data Insights 代理概觀](https://experienceleague.adobe.com/zh-hant/docs/platform-learn/tutorials/ai-assistant/data-insights-agent-overview) | Data Insights 代理能匯入事件、輪廓、歷程和離線資料，簡化探索工作，並立即啟用個人化儀表板。 | 2025 年 10 月 |
| [!DNL Experience Platform] | [Journey 代理概觀](https://experienceleague.adobe.com/zh-hant/docs/platform-learn/tutorials/ai-assistant/journey-agent-overview) | Journey 代理使用 AI，透過對話式介面建立、分析和最佳化 B2B/B2C 歷程。 | 2025 年 10 月 |
| [!DNL Experience Platform] | [Experimentation 代理概觀](https://experienceleague.adobe.com/zh-hant/docs/platform-learn/tutorials/ai-assistant/experimentation-agent-overview) | Experimentation 代理為行銷人員、分析師和管理員提供洞察、引導式設定和自動化分析，加速完成數位最佳化。 | 2025 年 10 月 |
| [!DNL Experience Platform] | [Audience 代理概觀](https://experienceleague.adobe.com/zh-hant/docs/platform-learn/tutorials/ai-assistant/audience-agent-overview) | Audience 代理提供對話式客群建立、最佳化、獲取洞察的功能，並安全地整合各個 B2B/B2C 應用程式，藉此簡化客群管理。 | 2025 年 10 月 |
| Acrobat | [利用 PDF 空間提升團隊效率](https://experienceleague.adobe.com/zh-hant/docs/document-cloud-learn/acrobat-learning/getting-started/pdf-spaces-legal) | 透過個人化的 AI 助理，將檔案和網站的集合轉換為可共用、可對話互動的知識中心。 | 2025 年 10 月 |

+++

## [!DNL Adobe System Status] {#status}

[!DNL Adobe System Status] 提供有關 Adobe 產品和服務的詳細資訊、狀態更新和電子郵件通知。收到有關服務中斷、作業中斷和維護事件的通知。請前往 [status.adobe.com](https://status.adobe.com/) 了解詳情。

+++發行說明

發行日期： **2025年12月8日**

**最新資訊**

* 虛擬助理增強功能
* 錯誤修正與改善功能

| 功能 | 說明 |
| ------- | ------- |
| **虛擬助理意見反應** | <ul><li>_虛擬小幫手_&#x200B;可協助Status的網頁和行動客戶執行快速動作並提供意見反應。 它是解析所有查詢的單一位置。</li><li>虛擬助理可供所有status.adobe.com的登入客戶使用。 登入status.adobe.com並使用&#x200B;_詢問我們的助理_ Widget即可存取它。</li><li>在此版本中，我們透過簡化工作流程、使用直覺式圖示而非按鈕，以及在虛擬助理或背景頁面上提供儘可能多的資訊，進一步增強引導式工作流程體驗。</li><li>如果您想要參與即將推出的AI虛擬助理的客戶測試版，請傳送電子郵件給[statuscom-adobe-support](mailto:statuscom-adobe-support@adobe.com)。</li></ul> |
| **錯誤修正和改善功能** | <ul><li>公告事件橫幅上的公告錯誤標示為&#x200B;_維護_。</li><li>有些Business Service名稱不太好用。</li><li>客戶無法檢視超過40天仍開啟的CSO，或40天前在新更新後關閉的CSO。</li><li>系統已通知未完成Slack設定的Adobe Status訂閱者，並提供完成安裝的步驟。</li><li>網站頁首和頁尾的效能改善。</li></ul> |

[!DNL Adobe System Status] 的舊版發行說明：

* [2025 年 8 月](https://experienceleague.adobe.com/zh-hant/docs/release-notes/experience-cloud/previous/2025/08132025#status)

+++

## [!DNL Experience Cloud] 管理與中央介面 {#ecloud}

了解 [!DNL Experience Cloud] 管理與中央介面元件的更新。

+++發行說明

| 日期 | 更新 | 說明 | 適用的產品 |
| -----------| -----------| ---------- | ---------- |
| **2026年1月14日** | 可自訂的左側導覽功能表 | 您現在可以在Experience Cloud中自訂應用程式的左側導覽功能表。 重新排序導覽專案並控制其可見性，以快速存取最重要的功能。 此更新可幫助您簡化工作流程、減少認知負擔，並支援完整的鍵盤協助工具。 | <ul><li>Experience Platform</li><li>Journey Optimizer</li><li>Journey Optimizer B2B 版</li><li>Dynamic Chat</li></ul> |

請參閱 [Experience Cloud 介面和管理](https://experienceleague.adobe.com/zh-hant/docs/core-services/interface/experience-cloud)指南，並獲得以下方面的協助：

<!-- * [Generative AI in Experience Cloud applications](https://experienceleague.adobe.com/en/docs/core-services/interface/features/generative-ai)
* [User management and product licenses](https://experienceleague.adobe.com/en/docs/core-services/interface/administration/admin-console) (Admin Console)
* [Customer Attributes, Audience Library, Assets](https://experienceleague.adobe.com/en/docs/core-services/interface/services/overview), and more -->

+++

## [!DNL Experience Platform] {#platform}

尋找 [!DNL Experience Platform] 和 [!UICONTROL Mobile SDK] 的最新版本資訊和新文件。查看新的教學課程與知識庫支援文章。

+++檢視更新

**Experience Platform**

**2025年10月22日**&#x200B;更新：如需詳細資訊，請參閱[[!DNL Experience Platform] 發行說明](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/release-notes/latest)。

**Mobile SDK**

更新日期： **2026年1月7日**

如需詳細資訊，請參閱 [[!DNL Experience Platform] Mobile SDK 發行說明](https://developer.adobe.com/client-sdks/documentation/release-notes/)。

<!-- ### New [!DNL Experience Platform] tutorials{#tutorials-aep}

New tutorials published for Adobe [!DNL Experience Platform] on Experience League.

| Published | Applications | Name | Type | Description |
| ----------| ---------- | ---------- | ---------- |---------- |
|November 2025| [!DNL Experience Platform] | [Use the Adobe Experience Platform Agent Orchestrator interface](https://experienceleague.adobe.com/en/docs/platform-learn/tutorials/ai-assistant/agent-orchestrator-ui)| Video | Learn how to interact with Experience Cloud solutions through contextual prompts, dynamic objects, and multiple specialized agents using Adobe Experience Platform Agent Orchestrator.  | -->

### 新的 [!DNL Experience Platform] 支援知識庫{#kb-aep}

[!DNL Experience Platform] 的新文章和現有文章的更新。

| 發佈日期 | 名稱 | 類型 | 說明 |
|---------|----|----|-----------|
| 2025 年 12 月 | [在Adobe Experience Platform中如何處理隱私權存取和刪除要求](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29736) | 新文章 | 瞭解如何在Adobe Experience Platform中處理隱私權存取和刪除請求。 |
| 2025 年 12 月 | [自訂CSV上傳30天後對象母體下降](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29670) | 新文章 | 瞭解當在Adobe Experience Platform (AEP)中上傳CSV檔案所建立的對象在30天後合格設定檔可能會急劇減少時問題的解決方案。 |
| 2025 年 12 月 | [將Power BI連線到Customer Journey Analytics AEP時發生認證驗證錯誤](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29668) | 新文章 | 瞭解當使用SQL使用者端在AEP中將Power BI連線至Adobe Customer Journey Analytics中的BI Connector時發生驗證錯誤時，該問題的解決方案。 |
| 2025 年 12 月 | [Adobe Experience Platform中的XDM ExperienceEvent資料集不支援更新](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29689) | 新文章 | 瞭解當資料流嘗試在Adobe Experience Platform中的XDM ExperienceEvent資料集上執行更新或更新插入作業時，發生錯誤&#x200B;*事件*&#x200B;不支援更新時問題的解決方案。 |
| 2025 年 12 月 | [*啟用對象*&#x200B;時， [!DNL Azure Blob] 目的地上的匯出訂單未包含有效的目的地ID](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29708)錯誤 | 新文章 | 瞭解在AEP中設定[!DNL Azure Blob]連線後，如果尚未授予必要的存取許可權，則啟用區段時會發生錯誤，此問題的解決方案。 |

+++

## [!DNL Real-Time CDP] {#rtcdp}

尋找 [!DNL Real-Time CDP] 的最新教學課程。

+++檢視新的教學課程

<!-- | Published | Name | Type | Description |
| ----------| ---------- | ---------- |---------- |
| October 2025 | [Configure a social destination](https://experienceleague.adobe.com/en/docs/platform-learn/tutorials/destinations/configure-a-social-destination#destinations) |  Video | Learn how to connect and activate audiences to a social destination. This video demonstrates the workflow using a LinkedIn Matched Audiences destination in Adobe Real-Time Customer Data Platform.| -->

如需說明，請參閱：

* 教學影片：[了解 Real-Time Customer Data Platform](https://experienceleague.adobe.com/zh-hant/docs/platform-learn/tutorials/rtcdp/understanding-the-real-time-customer-data-platform)

* 產品文件：[Real-Time Customer Data Platform](https://experienceleague.adobe.com/zh-hant/docs/real-time-customer-data-platform)

+++

## [!DNL Analytics] {#analytics}

尋找 [!DNL Adobe Analytics] 和 [!DNL AppMeasurement] 的最新版本資訊。查看新的教學課程與支援文章。

+++發行說明和新的教學課程

Adobe Analytics 版本會在持續傳遞模式上運作，允許以更可擴展且分階段的方法進行功能部署。發行說明每月會有多次更新。

更新： **2026年1月15日**

如需詳細資訊，請參閱[[!DNL Analytics] 發行說明](https://experienceleague-review.corp.adobe.com/docs/analytics/release-notes/latest.html)。

### AppMeasurement {#appm}

如需詳細資訊，請參閱 [JavaScript 適用的 AppMeasurement 發行說明](https://github.com/adobe/appmeasurement/releases)。

<!-- ### New Analytics tutorials {#tutorials-analytics}

New or updated video tutorials published for Adobe Analytics.

|Published|Name|Type|Description |
| -----------| ---------- | ---------- | ---------- |
|November 2025|[Classification sets job history](https://experienceleague.adobe.com/en/docs/analytics-learn/tutorials/components/classifications/classification-sets-job-history)| Video |See your job history of uploading and downloading classification set data.|
|November 2025|[Classification sets data import methods](https://experienceleague.adobe.com/en/docs/analytics-learn/tutorials/components/classifications/classification-sets-data-import-methods)| Video |Learn different methods of importing classification data with classification sets, as well as some associated use cases.| -->

<!--
### New [!DNL Analytics] support knowledge base{#kb-analytics}

New articles and updates to existing articles for [!DNL Analytics].

|Published|Name|Type|Description|
|---------|--------|---------|---------|
|November 2025|[Why does the **[!UICONTROL Workspace]** **[!UICONTROL Page]** dimension show over 100 bytes (page length exceeds 100-bytes limit)?](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28067)|New article| Learn about the solutions to the issue when a page listed in the **[!UICONTROL Page]** dimension appeared to have a string value exceeding 100 bytes, even though the page dimension has a 100-byte limit.|
-->

+++

## [!DNL Customer Journey Analytics] {#cja}

尋找 [!DNL Customer Journey Analytics] 的最新版本資訊。檢視 Experience League 上的全新教學課程。

+++發行說明和新的教學課程

[!DNL Customer Journey Analytics] 會持續發行新版本。因此，發行說明每月會更新多次。請定期查看。

更新： **2026年1月15日**

請參閱 [Customer Journey Analytics (CJA) 發行說明](https://experienceleague-review.corp.adobe.com/docs/analytics-platform/using/releases/latest.html#releases)以了解詳細資訊。

### [!DNL Customer Journey Analytics] 最新教學課程 {#tutorials-cja}

全新的 [!DNL Customer Journey Analytics] 教學課程

| 發佈日期 | 名稱 | 類型 | 說明 |
| -----------| ---------- | ---------- | ---------- |
| 2026 年 1 月 | [CJA銜接啟用與驗證](https://experienceleague.adobe.com/en/docs/customer-journey-analytics-learn/tutorials/visitor-id/stitching-enablement-and-validation) | 影片 | 瞭解如何在任何事件資料集上啟用彙整功能。 |
| 2026 年 1 月 | [追蹤和分析AI流量](https://experienceleague.adobe.com/en/docs/customer-journey-analytics-learn/tutorials/use-cases/ai/track-and-analyze-ai-traffic) | 影片 | 瞭解如何使用衍生的欄位、區段和Adobe Customer Journey Analytics專案，在Workspace中識別和篩選AI產生的流量，以確保客戶獲得準確、以人為中心的見解。 |

<!--
### New [!DNL Customer Journey Analytics] support knowledge base{#kb-cja}

New articles and updates to existing articles for [!DNL Customer Journey Analytics].

|Published|Name|Type|Description|
|---------|----|----|-----------|
|July 2025|[Access issues with shared metrics and dimensions in CJA](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27020)|New article| Learn about the solutions to the issue in Adobe Customer Journey Analytics where you can't access **[!UICONTROL Shared Metrics and Dimensions]**.|
-->

+++

## [!DNL Streaming Media Analytics] {#sma}

尋找 [!DNL Streaming Media Analytics] 的最新版本資訊。檢視 Experience League 上的全新教學課程。

+++發行說明

更新日期：**2025 年 10 月**

* [!DNL Streaming Media Analytics] [發行說明](https://experienceleague.adobe.com/zh-hant/docs/media-analytics/using/release-notes/release-notes)

* [!DNL Streaming Media Analytics] [產品文件與教學課程](https://experienceleague.adobe.com/zh-hant/docs/media-analytics/using/media-overview)

+++

## [!DNL Experience Manager] {#aem}

[!DNL Experience Manager] 中的新功能、修正及更新。Adobe 建議使用內部部署的客戶部署最新修補程式，以確保擁有更出色的穩定性、安全性及效能。

+++發行說明、新的教學課程，以及支援文章

### Experience Manager as a Cloud Service 發行說明

新增： AEM中的[Agentic AI](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/overview?#agentic-ai-in-aem)

下次預計發行日期： **2026.1.0 - 2026年1月29日**

* [AEM CS 發行說明首頁](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-cloud-service/content/release-notes/release-notes/release-notes-current)
* [最新 AEM CS 版本概觀影片](https://experienceleague.adobe.com/zh-hant/docs/events/aemcs-release-update-recordings/overview)

其他 [!DNL Experience Manager] 的產品發行說明都會保留在以下頁面：

* [[!DNL Experience Manager] 6.5 Service Pack 發行說明](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-65/content/release-notes/release-notes)
* [[!DNL Experience Manager] Cloud Manager 發行說明](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-cloud-manager/content/release-notes/current)
* [Automated Forms Conversion Service 發行說明](https://experienceleague.adobe.com/zh-hant/docs/aem-forms-automated-conversion-service/using/release-notes)
* [[!DNL Experience Manager] Assets Dynamic Media 發行說明](https://experienceleague.adobe.com/zh-hant/docs/dynamic-media-developer-resources/release-notes/s7rn2017)
* [[!DNL Experience Manager] Brand Portal 發行說明](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-brand-portal/using/introduction/brand-portal-release-notes)
* [[!DNL Experience Manager] 桌面應用程式發行說明](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-desktop-app/using/release-notes)
* [[!DNL Experience Manager] Dispatcher 發行說明](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-dispatcher/using/getting-started/release-notes)

### [!DNL Experience Manager] 最新教學課程 {#tutorials-aem}

專為 [!DNL Experience Manager as a Cloud Service] 發佈的新影片教學課程和書面教學課程 (文章)。

| 發佈日期 | 應用程式 | 名稱 | 格式 | 說明 |
| ----------| ---------- | ---------- | ---------- |---------- |
| 2026 年 1 月 | [!DNL AEM Assets] | [Dynamic Media URL語法和影像預設集最佳作法](https://experienceleague.adobe.com/en/docs/experience-manager-learn/assets/dynamic-media/images/dynamic-media-urls-and-image-presets) | 影片 | 探索AEM Assets Dynamic Media URL語法，瞭解如何使用影像預設集將您的價值最大化。 增強您的資產管理功能並提高工作流程效率。 |
| 2026 年 1 月 | [!DNL AEM Assets] | [在AEM Assets Content Hub中下載資產](https://experienceleague.adobe.com/en/docs/experience-manager-learn/assets/content-hub/download-assets) | 影片 | 瞭解如何導覽、搜尋和下載Adobe Experience Manager Assets Content Hub中的資產。 |
| 2026 年 1 月 | [!DNL AEM CS] | AEM中的[代理程式](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/ai/agents-in-aem) | 影片 | 瞭解AEM代理程式如何透過在Adobe Experience Manager中自動化任務來簡化工作流程。 |

### 新的 [!DNL Experience Manager] 支援知識庫{#kb-aem}

[!DNL Experience Manager] 的新文章和現有文章的更新。

| 發佈日期 | 名稱 | 類型 | 說明 |
|---------|--------|---------|---------|
| 2025 年 12 月 | [Dynamic Media Scene7快取存留時間問題](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29687) | 新文章 | 瞭解Dynamic Media Scene7快取存留時間設定。 |
| 2025 年 12 月 | [無法載入特定區域中使用者子集的Dynamic Media影像](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29709) | 新文章 | 瞭解當部分使用者無法正常載入某些URL至Dynamic Media時，其他使用者無法正常載入這些URL的問題的解決方案。 |
| 2025 年 12 月 | 在AEM中更新集合後，[Brand Portal共用連結未顯示任何資產](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28072) | 新文章 | 瞭解當共用集合連結在新增或更新資產後顯示&#x200B;*沒有可顯示的內容*&#x200B;時，AEM Brand Portal中問題的解決方案。 |
| 2025 年 12 月 | [在AEM 6.5.23中設定 [!DNL Microsoft Translator] 但沒有端點URL欄位](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28099) | 新文章 | 瞭解將[!DNL Microsoft Translator]與Adobe Experience Manager (AEM) 6.5.23整合時，設定對話方塊未提供可輸入Microsoft端點URL的欄位。 |
| 2025 年 12 月 | [由於AEM Dynamic Media中的編碼損毀，視訊播放失敗](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29548) | 新文章 | 瞭解在Adobe Experience Manager (AEM) Dynamic Media中，當視訊播放由於某些視訊資產的編碼損毀而中途停止時，該問題的解決方案。 |
| 2025 年 12 月 | [在AEM Dynamic Media中有效率地重新處理HLS/DASH的大量視訊資產](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29601) | 新文章 | 瞭解在AEM Dynamic Media中為HLS/DASH串流重新處理視訊資產時，系統過載並導致大量執行時失敗問題的解決方案。 |
| 2025 年 12 月 | [JSON檔案會在AEM中觸發不需要的DAM更新資產工作流程](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29606) | 新文章 | 瞭解當將JSON檔案上傳至AEM並觸發DAM更新資產工作流程，導致建立不需要的處理及轉譯時，Assets中問題的解決方案。 |
| 2025 年 12 月 | [為什麼您會在AEM記錄檔中看到&#x200B;*切換`FT_GRANITE-61513`已停用*](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29639) | 新文章 | 瞭解在AEM as a Cloud Service中發佈時問題的解決方案，您會看到指出`FT_GRANITE-61513`已停用的記錄檔。 |
| 2025 年 12 月 | [Adobe Experience Manager：升級到AEM 6.5 LTS SP1](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29732)後，許多索引更新訊息會重新出現 | 新文章 | 瞭解此問題的解決方案：升級至AEM 6.5 LTS SP1後，先前未顯示的索引記錄訊息會在升級後重新顯示。 |
| 2025 年 12 月 | [Adobe Experience Manager：無法儲存啟動內容與&#x200B;*無法在AEM as a Cloud Service*&#x200B;中提交表單](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29733) | 新文章 | 瞭解當您嘗試儲存已新增至Launch的內容，但變更未成功儲存時，出現錯誤訊息&#x200B;*無法提交表單*&#x200B;時，AEMaaCS中問題的解決方案。 |
| 2025 年 12 月 | 升級至AEM 6.5 SP23[後，`allowedComponents`中的`model.json`遺失](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28057) | 新文章 | 瞭解當升級至Service Pack 23後，`allowedComponents`中繼資料不再包含在Publish執行個體的`model.json`匯出中時，AEM中問題的解決方案。 |
| 2025 年 12 月 | [正在解決AEM Publish執行個體中的循環&#x200B;*SegmentNotFoundException*&#x200B;錯誤](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28172) | 新文章 | 瞭解當週期性&#x200B;*SegmentNotFoundException*&#x200B;錯誤導致Adobe Experience Manager (AEM) Publish執行個體當機並停止回應時，問題的解決方案。 |
| 2025 年 12 月 | [覆寫AEM `AdaptiveImageServletMappingConfigurationFactory`以支援WebP](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29598) | 新文章 | 瞭解Adobe Experience Manager (AEM)的影像元件除非`AdaptiveImageServletMappingConfigurationFactory`設定包含WebP擴充功能，否則不會轉譯WebP資產時問題的解決方案。 |
| 2025 年 12 月 | [解決AEM 6.5](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29651)中許可權導致的標籤移動錯誤 | 新文章 | 瞭解將標籤從一個名稱空間移動到另一個名稱空間，然後移動到其原始位置時問題的解決方案，操作失敗並出現&#x200B;*拒絕存取*。 |
| 2025 年 12 月 | [在AEM中發佈父頁面之前，不會顯示子頁面變更](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29671) | 新文章 | 瞭解當對子頁面進行的變更未出現在即時網站上（除非同時發佈父頁面）時，AEM Managed Services中問題的解決方案。 |
| 2025 年 12 月 | [取消發佈語言主版頁面失敗，因為AEM中的子頁面已鎖定](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29650) | 新文章 | 瞭解取消發佈語言主版頁面因AEM中鎖定的子頁面而失敗的問題解決方案。 |
| 2025 年 12 月 | 由於PDF安全性設定[，*PDF Generator中的Acrobat匯出失敗，錯誤為* ALC-PDG-010-011](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28137) | 新文章 | 瞭解在[!DNL Word]上使用AEM Forms 6.5.23中的PDF Generator將PDF匯出為[!DNL Excel]、[!DNL Windows Server 2022]或其他格式時，問題的解決方案，流程失敗並出現錯誤&#x200B;*ALC-PDG-010-011*，表示無法連絡PDFMaker。 |
| 2025 年 12 月 | [解決AEM 6.5 (Forms和其他解決方案)中 [!DNL Groovy Console] 稽核追蹤所造成的區段存放區成長問題](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28178) | 新文章 | 瞭解當您的AEM 6.5內部部署或AMS環境顯示突然的磁碟尖峰和快速成長的[!UICONTROL TarMK]區段存放區時，問題的解決方案。 |
| 2025 年 12 月 | 在AEM Forms中啟動Forms期間[序列化失敗](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29676) | 新文章 | 瞭解在升級至AEM Forms後，伺服器啟動失敗且序列化失敗時問題的解決方案。 |
| 2025 年 12 月 | [AEM Sites中內嵌指令碼的內容安全性原則Nonce支援](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29587) | 新文章 | 瞭解AEM Sites是否支援CSP Nonce或安全替代方案，以便載入沒有不安全指示的內嵌指令碼。 |
| 2025 年 12 月 | 在AEM as a Cloud Service中設定重寫環境時發生[授權錯誤](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27823) | 新文章 | 瞭解當在Adobe Experience Manager (AEM) as a Cloud Service a *403禁止的[管理員]未授權*&#x200B;中為EDS/xwalk專案設定無原則環境時，問題的解決方案。 |
| 2025 年 12 月 | [來自Dynamic Media或AEM Assets的Assets未出現在Content Hub](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28074)中 | 新文章 | 瞭解當AEM as a Cloud Service中的Content Hub不顯示來自Dynamic Media或AEM Assets的資產（即使在發佈和核准後）時問題的解決方案。 |
| 2025 年 12 月 | [唯一欄位驗證因AEM內容片段中的自訂地區設定資料夾而失敗](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28085) | 新文章 | 瞭解當AEM中的自訂地區設定資料夾名稱無法辨識為語言根目錄而導致唯一欄位驗證錯誤時，該問題的解決方案。 |
| 2025 年 12 月 | [AEM as a Cloud Service中的使用者節點損毀所導致的登入失敗](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28127) | 新文章 | 瞭解當Adobe Experience Manager (AEM) as a Cloud Service中的特定使用者，在其使用者節點包含損壞或不正確的資料（例如無效的電子郵件地址）時，登入失敗問題的解決方案。 |
| 2025 年 12 月 | [工作流程鈴鐺圖示未顯示AEM中Admin Console使用者的本機任務](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28140) | 新文章 | 瞭解當本機工作流程任務（例如參與者步驟核准）未出現在Admin Console同步使用者的鈴鐺圖示中（即使任務會顯示在收件匣中），問題的解決方案。 |
| 2025 年 12 月 | 在AEMaaCS中重新整理後，[Dynamic Media版面容器在編輯模式中消失](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28171) | 新文章 | 瞭解在Adobe Experience Manager as a Cloud Service (AEMaaCS)中使用Dynamic Media核心元件進行視訊製作時，版面容器在重新整理後，會從編輯模式的頁面中消失的問題的解決方案。 |
| 2025 年 12 月 | [401在AEM as a Cloud Service](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28177)上進行功能測試時發生未獲授權的錯誤 | 新文章 | 瞭解當AEM as a Cloud Service功能測試期間發生401 Unauthorized錯誤，因為Dispatcher使用不支援的基本驗證時，此問題的解決方案。 |
| 2025 年 12 月 | 當[寄件者&#x200B;**[!UICONTROL 位址在Adobe Experience Manager as a Cloud Service]**&#x200B;中不同時，](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29609)工作流程電子郵件無法傳遞 | 新文章 | 瞭解Day CQ郵件服務中的&#x200B;**[!UICONTROL 寄件者]**&#x200B;地址與Day CQ工作流程電子郵件通知服務中的地址不符時，工作流程電子郵件通知無法傳送時，AEMaaCS中問題的解決方案。 |
| 2025 年 12 月 | [由於AEMaaCS中的許可權不足，無法使用內容片段發佈DAM資料夾](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29652) | 新文章 | 瞭解當在Adobe Experience Manager as a Cloud Service (AEMaaCS)中發佈包含內容片段的DAM資料夾失敗並出現許可權不足，無法管理發布的錯誤時，該問題的解決方案。 |
| 2025 年 12 月 | AEMaaCS中核准的資產缺少[傳遞URL](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28098) | 新文章 | 瞭解當具有OpenAPI的Dynamic Media啟用時，Adobe Experience Manager as a Cloud Service (AEMaaCS)中已核准和發佈資產的傳送URL未出現，但使用者許可權遺失時的問題解決方案。 |
| 2025 年 12 月 | [403禁止使用已編碼斜線的URL](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28182) | 新文章 | 瞭解當URL包含編碼斜線(`%2F`)時，Adobe Experience Manager as a Cloud Service傳回403禁止錯誤。 |

+++

## [!DNL Adobe LLM Optimizer] {#llm-optimizer}

尋找 LLM Optimizer 的相關說明，這是 Adobe 針對生成式引擎最佳化推出的生成式 AI 優先應用程式。

+++最新教學課程

| 發佈日期 | 名稱 | 類型 | 說明 |
| -----------| ---------- | ---------- | ---------- |
| 2025 年 11 月 | [LLM Optimizer最佳實務](https://experienceleague.adobe.com/en/docs/llm-optimizer/using/essentials/best-practices) | 影片 | 探索LLM最佳化的最佳實務，以提升AI搜尋中的品牌可見度。 基準測試與最佳化內容的深入分析。 |
| 2025 年 11 月 | [設定類別、主題、提示等的最佳實務](https://experienceleague.adobe.com/en/docs/llm-optimizer/using/essentials/best-practices-topics-prompts) | 影片 | 透過設定類別、主題、提示和其他品牌來追蹤（包括量身打造的品牌監控和策略內容分析的競爭對手），最佳化LLM深入分析。 |

+++

## [!DNL Brand Concierge] {#brand-concierge}

尋找[!DNL Brand Concierge]的最新教學課程和影片。

+++最新教學課程

| 發佈日期 | 名稱 | 類型 | 說明 |
| -----------| ---------- | ---------- | ---------- |
| 2026 年 1 月 | [預約會議](https://experienceleague.adobe.com/en/docs/brand-concierge/content/getting-started/meeting-booking) | 影片 | 瞭解如何預約會議。 以客戶身分檢視體驗，並設定代表使用其行事曆和檢視報告 |

如需詳細資訊，請造訪[Adobe Brand Concierge](https://experienceleague.adobe.com/en/docs/brand-concierge/content/home)教學課程首頁。

+++

## [!DNL Commerce] {#commerce}

存取 [!DNL Adobe Commerce] 的發行說明、新的教學課程和知識庫支援文章。

+++發行說明、新的教學課程，以及支援文章

* 請參閱 [&#x200B; [!DNL Adobe Commerce]  和  [!DNL Magento Open Source] 發行說明](https://experienceleague.adobe.com/zh-hant/docs/commerce-operations/release/notes/overview)以取得最新資訊。
* 請參閱「[Adobe [!DNL Commerce] 服務指南](https://experienceleague.adobe.com/zh-hant/docs/commerce/user-guides/home)」，查看 Commerce 服務版本資訊和文件。
* 請參閱[產品可用性](https://experienceleague.adobe.com/zh-hant/docs/commerce-operations/release/product-availability)以存取個別的產品發行說明並確認可用性。

### 全新的 [!DNL Adobe Commerce] 教學課程 {#tutorials-commerce}

在 Experience League 上尋找 [!DNL Adobe Commerce] 的新教學課程。

| 發佈日期 | 名稱 | 類型 | 說明 |
| -----------| ---------- | ---------- | ---------- |
| 2026 年 1 月 | [可觀察性概觀](https://experienceleague.adobe.com/en/docs/commerce-learn/tutorials/observability/overview) | 影片 | 瞭解集中式可觀察性為何對現代Adobe Commerce架構至關重要。 |
| 2026 年 1 月 | [開啟遙測](https://experienceleague.adobe.com/en/docs/commerce-learn/tutorials/observability/open-telemetry) | 影片 | 瞭解開放式遙測如何標準化跨記錄、追蹤和量度的可觀察性，而不會將您鎖定在單一監視工具中。 |
| 2026 年 1 月 | [安裝程式和組態](https://experienceleague.adobe.com/en/docs/commerce-learn/tutorials/observability/demo-setup-configuration) | 影片 | 瞭解如何使用標準化、相關的遙測技術，實現跨Adobe Commerce和自訂應用程式的端對端可觀察性。 |
| 2026 年 1 月 | [可觀察性示範](https://experienceleague.adobe.com/en/docs/commerce-learn/tutorials/observability/demo) | 影片 | 瞭解Adobe Commerce如何使用可觀察性、webhook和事件關聯來順暢地追蹤和偵錯。 |

### 新的 [!DNL Commerce] 支援知識庫{#kb-commerce}

Adobe Commerce 的新文章和現有文章的更新。

| 發佈日期 | 名稱 | 類型 | 說明 |
|---------|--------|---------|---------|
| 2025 年 12 月 | [[!DNL Quality Patches Tool] (QPT)](https://experienceleague.adobe.com/zh-hant/docs/commerce-operations/tools/quality-patches-tool/patches-available-in-qpt/patches-available-in-qpt-tool-overview) | 新文章 | 有關如何套用QPT 1.1.74中可用修補程式的新文章已發佈，可在其各自的章節中找到。 |
| 2025 年 12 月 | [指派給多個組織之使用者的Adobe Identity Management Service (IMS)登入問題](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29604) | 新文章 | 瞭解當屬於多個Adobe IMS組織的Adobe Commerce使用者在登入期間選取錯誤的組織時登入失敗的問題解決方案。 |
| 2025 年 12 月 | [[!DNL Fastly] 快取無法在雲端基礎結構上的Adobe Commerce上運作](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29629) | 新文章 | 瞭解當[!DNL Fastly]快取無法在您的網站上運作時問題的解決方案。 |
| 2025 年 12 月 | [失效的快取導致回應時間降低](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29631) | 新文章 | 瞭解如何防止快取失效，因為快取失效會降低Adobe Commerce存放區效能。 |
| 2025 年 12 月 | [Adobe Commerce上的資料庫儲存疑難排解員](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29633) | 新文章 | 瞭解Adobe Commerce中資料庫儲存問題的解決方案。 |
| 2025 年 12 月 | [Adobe Commerce部署疑難排解員](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29640) | 新文章 | 瞭解Adobe Commerce中發生停滯部署和失敗部署時的問題解決方案。 |
| 2025 年 12 月 | [無法儲存實體Adobe Commerce後端](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29642) | 新文章 | 瞭解當您無法在Adobe Commerce後端儲存實體時，問題的解決方案。 |
| 2025 年 12 月 | [匯入CSV後客戶格線中不顯示新客戶](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29643) | 新文章 | 瞭解從&#x200B;**[!UICONTROL 檔案匯入後，當您在]**&#x200B;客戶&#x200B;**[!UICONTROL >]**&#x200B;所有客戶`.csv`下看不到新客戶時，此問題的解決方案。 |
| 2025 年 12 月 | [疑難排解Adobe Commerce的`/tmp`掛載已滿](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29644) | 新文章 | 瞭解當`/tmp`掛載已滿、網站可能已關閉，且您無法透過SSH連線至節點時，此問題的解決方案。 |
| 2025 年 12 月 | [無法移除原始碼或變更原始碼](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29648) | 新文章 | 瞭解當您無法刪除來源或修改其程式碼，因為系統限制這些動作時，問題的解決方案。 |
| 2025 年 12 月 | [修正&#x200B;*類別無法儲存在產生的/程式碼目錄中*&#x200B;錯誤](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29649) | 新文章 | 瞭解指定相依性的方式會阻止類別即時自動產生，且無法將&#x200B;*類別儲存在產生的/程式碼目錄*&#x200B;錯誤訊息中的問題的解決方案。 |
| 2025 年 12 月 | [Adobe Commerce [!DNL Fastly] 疑難排解員](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29661) | 新文章 | 此[!DNL Fastly]Adobe Commerce使用者疑難排解員會根據您對發生症狀的回應，引導您找到解決方案。 |
| 2025 年 12 月 | [清除雲端上的 [!DNL Fastly] 快取時發生錯誤（*未成功處理清除要求。*）](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29667) | 新文章 | 瞭解使用[!DNL Fastly]清除選項且發生錯誤時問題的解決方案： *未成功處理清除請求。* |
| 2025 年 12 月 | 儲存包含1k+個產品的類別時，[504閘道逾時錯誤](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29703) | 新文章 | 瞭解對大型類別（1,000多種產品）執行作業時逾時問題的解決方案。 |


+++

## [!DNL Target] {#target}

取得 [!DNL Adobe Target] 的預發行說明、最新發行說明和新教學課程的存取權。

+++發行說明

發行日期：**2025年11月14日**

<!-- ### New [!DNL Target] support knowledge base{#kb-target}

|Published|Name|Type|Description|
|---------|----|----|-----------|
|July 2024|[[!DNL Adobe Target] bulk profile update [!DNL API] throws *[!DNL Unexpected Error]* when using [!DNL Postman]](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-24281)|New article| Learn about how to fix the issue when running the [!DNL Target Bulk Profile Update API] throws the *[!DNL Unexpected Error]* message in [!DNL Postman].|
-->

* 如需搶鮮版資訊，請參閱 [[!DNL Adobe Target]  搶鮮版](https://experienceleague.adobe.com/zh-hant/docs/target/using/release-notes/target-release-notes)
* 如需目前版本資訊，請參閱 [[!DNL Adobe Target]  發行說明](https://experienceleague.adobe.com/zh-hant/docs/target/using/release-notes/release-notes)

+++

## [!DNL Campaign] {#ac}

取得 [!DNL Adobe Campaign] 最新更新。在 Experience League 上尋找新教學課程和知識庫支援文章。

+++發行說明和支援文章

### 最新 Campaign 產品版本

* [!DNL Web User Interface]： 2025年10月25日 — [發行說明](https://experienceleague.adobe.com/zh-hant/docs/campaign-web/v8/release-notes/release-notes) | [產品檔案](https://experienceleague.adobe.com/zh-Hant/docs/campaign-web/v8/campaign-web-home)

* [!DNL Campaign]v8：2025 年 10 月 9 日 - [發行說明](https://experienceleague.adobe.com/zh-hant/docs/campaign/campaign-v8/releases/release-notes#release-8-7-4) | [產品文件](https://experienceleague.adobe.com/zh-hant/docs/campaign/campaign-v8/campaign-home)

* [!DNL Campaign Standard]：25.2 - 2025 年夏季 - [發行說明](https://experienceleague.adobe.com/zh-hant/docs/campaign-standard/using/release-notes/release-notes) | [產品文件](https://experienceleague.adobe.com/zh-hant/docs/campaign-standard/using/campaign-standard-home)

* [!DNL Campaign Classic] 7.4.2：2025 年 5 月 12 日 - [發行說明](https://experienceleague.adobe.com/zh-hant/docs/campaign-classic/using/release-notes/latest-release#release-7-4-2) | [產品文件](https://experienceleague.adobe.com/zh-hant/docs/campaign-classic/using/campaign-classic-home)

<!-- ### New [!DNL Campaign] tutorials {#tutorials-campaign}

New or updated videos tutorials published for Adobe Campaign.

| Published | Application | Name | Type | Description |
| ----------| ---------- | ---------- | ---------- |---------- |
|August 2025|Campaign v8 Web User Interface |[Enhance a delivery with dynamic content](https://experienceleague.adobe.com/en/docs/campaign-web-learn/tutorials/content-management/enhance-a-delivery-with-dynamic-content)| Video tutorial |Learn how to use the generative AI powered Content Generator to create and enhance the email content, optimizing your email deliveries to better resonate with your audience.|
|August 2025|Campaign v8 Web User Interface |[Configure JavaScript code activity](https://experienceleague.adobe.com/en/docs/campaign-web-learn/tutorials/workflows/configure-java-script-code-activity)| Video tutorial |Learn how to use the JavaScript Code activity in Adobe Campaign Web v8 to enhance your workflows. This tutorial covers both simple and advanced scripting modes. It explains execution settings and error handling. View real-world examples, such as targeting VIP profiles, transforming data, and triggering conditional operations. |
 -->

### 新的 [!DNL Campaign] 支援知識庫{#kb-campaign}

[!DNL Campaign] 的新文章和現有文章的更新。

| 發佈日期 | 名稱 | 類型 | 說明 |
|---------|----|----|-----------|
| 2025 年 12 月 | [`queryDef`在Adobe Campaign工作流程中僅傳回10,000筆記錄](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28003) | 新文章 | 瞭解在Adobe Campaign Classic中執行工作流程時問題的解決方案，如果您未指定`lineCount`屬性，則JavaScript活動內的`queryDef`僅傳回10,000筆記錄。 |
| 2025 年 12 月 | [由於Adobe Campaign Standard中的傳遞計數臨界值所導致，報告中的行銷活動可見性受限](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28077) | 新文章 | 瞭解在Adobe Campaign Standard中依行銷活動名稱篩選時，當某些業務使用者無法在一般&#x200B;**[!UICONTROL 報告]**&#x200B;區段中檢視特定每月行銷活動時，此問題的解決方案。 |
| 2025 年 12 月 | [外部帳戶清理會移除Adobe Campaign Classic中的清單表格](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28069) | 新文章 | 瞭解當收件者及非收件者清單的資料庫表格在Adobe Campaign Classic (ACC)環境中每天刪除時問題的解決方案。 |
| 2025 年 12 月 | [Campaign Standard：篩選行銷活動標籤的全域報告會為不同的使用者帶來不同的結果](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29728) | 新文章 | 瞭解為什麼不同角色的使用者在使用行銷活動名稱作為篩選條件來篩選&#x200B;**[!UICONTROL 全域報告]**&#x200B;時，會看到不同的數字。 |

+++

## [!DNL Journey Optimizer] {#journey-opt}

了解 [!DNL Journey Optimizer] 的最新版本資訊。在 Experience League 上檢視最新的教學課程和知識庫支援文章。

+++發行說明和新的教學課程

### [!DNL Journey Optimizer] 產品發行更新

最新發行日期： **2025年11月24日**

如需詳細資訊，請參閱[Journey Optimizer發行說明](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer/using/whats-new/release-notes)。 (**注意：** [B2B edition的Journey Optimizer](#journey-optimizer-b2b-edition)是獨立的產品。)

### [!DNL Journey Optimizer] 最新教學課程 {#tutorials-ajo}

在 Experience League 上發佈的全新 Adobe [!DNL Journey Optimizer] 教學課程。

| 發佈日期 | 應用程式 | 名稱 | 類型 | 說明 |
| ---------- | ---------- | ---------- | ---------- |---------- |
| 2026 年 1 月 | [!DNL Journey Optimizer] | [行動學習中心](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer-learn/mobile-learning-hub/overview) | 多頁教學課程 | 透過Adobe Journey Optimizer快速啟動或提升您的行動互動策略。 此行動學習中心可為開發人員、管理員、行銷人員和分析師提供設定傳入和傳出行動頻道所需的一切，並將他們緊密整合到強大的跨頻道行銷活動和歷程中。 |

### 新的 [!DNL Journey Optimizer] 支援知識庫{#kb-ajo}

[!DNL Journey Optimizer] 的新文章和現有文章的更新。

| 發佈日期 | 名稱 | 類型 | 說明 |
|---------|----|----|-----------|
| 2025 年 12 月 | Journey Optimizer的自訂動作回應中不支援[陣列](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28054) | 新文章 | 瞭解從Adobe Journey Optimizer (AJO)中的自訂動作叫用API時，問題的解決方案，回應會傳回陣列。 |


### [!DNL Journey Optimizer] 的更多資源

* [[!DNL Journey Optimizer] 文件](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer/using/ajo-home) - [發行說明](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer/using/whats-new/release-notes) - [操作說明影片](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer-learn/tutorials/overview)
* [決策管理文件](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer/using/decisioning/offer-decisioning/get-started-decision/starting-offer-decisioning) - [發行說明](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer/using/whats-new/release-notes) - [操作說明影片](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer-learn/tutorials/decision-capabilities/decision-management/introduction-to-decision-management) - [最新文件更新](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer/using/whats-new/documentation-updates)

+++

## [!DNL Journey Optimizer B2B Edition] {#ajo-b2b}

了解關於 [!DNL Journey Optimizer B2B Edition] 的最新發行資訊。

+++發行說明和文件

最新發行日期： **2025.10 - 2025年10月31日**

如需詳細資訊，請參閱 [AJO B2B Edition 發行說明](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer-b2b/user/release-notes)。

**AJO B2B 資源**

* [[!DNL Journey Optimizer B2B Edition]](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer-b2b)&#x200B;(全自助)
* [!DNL Journey Optimizer B2B Edition]的[產品文件](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer-b2b/user/guide-overview)
* [!DNL Journey Optimizer B2B Edition]的[影片概觀與教學課程](https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer-b2b-learn/tutorials/overview)
* [!DNL Journey Optimizer B2B Edition]的[產品描述與授權](https://helpx.adobe.com/tw/legal/product-descriptions/adobe-journey-optimizer-b2b.html#_blankl) 

<!-- New videos, tutorials, or courses published for Journey Optimizer B2B Edition.

|Published|Name|Type|Description |
| -----------| ---------- | ---------- | ---------- |
|February 2025|[Account Journeys](https://experienceleague.adobe.com/en/docs/journey-optimizer-b2b-learn/tutorials/account-journeys/introducing-account-journeys)|New videos |Visit the Account Journeys tutorial home. Learn about Account Journeys and how to use them to engage your target audience.|
|February 2025|[Use Case Playbook - Abandoned shopping cart](https://experienceleague.adobe.com/en/docs/journey-optimizer-learn/tutorials/use-cases/abandoned-cart)|New video |Learn how to implement the abandoned shopping cart use case, using the Playbook feature in Adobe Journey Optimizer.|
|February 2025|[Import and activate an audience by uploading a CSV file](https://experienceleague.adobe.com/en/docs/journey-optimizer-learn/tutorials/profiles-audiences-subscriptions/import-and-activate-an-audience-by-uploading-a-csv-file)|New video |Discover how to efficiently import and activate an audience by uploading a CSV file. Learn to personalize your content using enrichment attributes from the CSV file, ensuring a more tailored experience for your audience.| -->

+++

## [!DNL Marketo Engage] {#marketo}

了解 [!DNL Marketo Engage] 的最新發行說明和發行排程。

+++發行說明、教學課程與支援文章

### Marketo Engage 核心更新

最新發行日期：**2025 年 10 月**

* [最新發行說明](https://experienceleague.adobe.com/zh-hant/docs/marketo/using/release-notes/current)
* [!DNL Marketo Engage] [&#x200B; 發行排程](https://experienceleague.adobe.com/zh-hant/docs/marketo/using/release-notes/release-schedule)
* Dynamic Chat [發行說明](https://experienceleague.adobe.com/zh-hant/docs/marketo/using/release-notes/dynamic-chat) (2025 年 6 月 30 日)

<!-- ### New Marketo tutorials {#tutorials-marketo}

New tutorials published for Adobe Marketo.

|Published|Name|Type|Description |
| -----------| ---------- | ---------- | ---------- |
|March 2025|[Best practices to implement live chat](https://experienceleague.adobe.com/en/docs/marketo-learn/tutorials/dynamic-chat/live-chat-best-practices)| New video |Learn about the best practices to follow when you're implementing the live chat feature in Dynamic Chat.| -->

如需最新的產品文件，請參閱 [Marketo 產品文件](https://experienceleague.adobe.com/zh-hant/docs/marketo/using/home)首頁

<!-- ### New [!DNL Marketo] support knowledge base

New articles and updates to existing articles for [!DNL Marketo].

|Published|Name|Type|Description|
| -----------| ---------- | ---------- | ---------- |
|July 2025|[Marketo Measure touchpoints not syncing to Marketo Engage](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26692)|New article| Learn about the solutions to the issue when Marketo Measure touchpoints don't appear in Marketo Engage person records, even though they sync successfully to [!DNL Salesforce].| 
-->

+++

## [!DNL Workfront] {#workfront}

了解 [!DNL Adobe Workfront] 的最新發行說明。在 Experience League 上找到新教學課程。

+++發行說明和新的教學課程

### [!DNL Adobe Workfront] 更新

* [2026年第一季版本總覽](https://experienceleague.adobe.com/en/docs/workfront/using/product-announcements/product-releases/release-26-q1/26-q1-release-overview)

* [2025 年第四季發行版本概觀](https://experienceleague.adobe.com/zh-hant/docs/workfront/using/product-announcements/product-releases/release-25-q4/25-q4-release-overview)

關於所有版本資訊：

* 請參閱「[Adobe  [!DNL Workfront]  產品發行](https://experienceleague.adobe.com/zh-hant/docs/workfront/using/product-announcements/product-releases/product-releases)」頁面，以了解 [!DNL Workfront] 發行排程資訊與發行說明。

* 請參閱「[Adobe  [!DNL Workfront]  Fusion 發行活動概觀](https://experienceleague.adobe.com/zh-hant/docs/workfront-fusion/using/fusion-release-activity/fusion-release-activity)」了解有關 Fusion 的最新資訊。

### 全新 Adobe [!DNL Workfront] 教學課程  {#tutorials-workfront}

在 Experience League 上的全新 [!DNL Workfront] 教學課程和活動。

| 發佈日期 | 名稱 | 類型 | 說明 |
| -----------| ---------- | ---------- | ---------- |
| 2026 年 1 月 | [使用Adobe Workfront外掛程式來整合Creative Cloud](https://experienceleague.adobe.com/en/docs/workfront-learn/tutorials-workfront/integrations/adobe-creative-cloud/use-adobe-workfront-extensions-for-creative-cloud) | 影片 | 將Workfront與Creative Cloud整合。 您可以在Workfront中尋找並更新指派給您的工作，而不需離開Creative Cloud應用程式(Photoshop、XD、InDesign、Illustrator、Premiere Pro和After Effects)。 |

<!--
### New [!DNL Workfront] support knowledge base

New articles and updates to existing articles for [!DNL Workfront].

|Published|Name|Type|Description|
| -----------| ---------- | ---------- | ---------- |
|August 2025|[Calendar appears blank when owner is deactivated in Workfront](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27192)|New article| Learn about the solutions to the issue when the owner of a calendar is deactivated, the calendar becomes blank, and no tasks or events are visible.|
|August 2025|[Error appears when assigning resource managers in Workfront](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27251)|New article| Learn about the solutions to the issue when the error *Only users with a plan license can be added as resource managers* occurs when attaching a project template that includes resource managers without the required license type.|
|August 2025|[Issue to project conversion grants home group "view" access by default in Workfront](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27261)|New article| Learn about the solutions to the issue when converting an issue into a project using a template in Adobe Workfront, the creator's home group automatically receives view access to the resulting project.|
|August 2025|[Duplicate folders auto-created in Workfront via [!DNL Fusion] scenarios](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27287)|New article| Learn about the solutions to the issue when duplicate folders are automatically created in certain Adobe Workfront projects due to active [!DNL Fusion] connections.|
|August 2025|[Unable to report on specific approver actions in Adobe Workfront](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27298)|New article| Learn about the solutions to the issue when Adobe Workfront reports don't natively support isolating actions taken by specific approvers in multi-stage approval workflows.|
|August 2025|[Unable to attach template to an existing project in Workfront](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27305)|New article| Learn about the solutions to the issue when an error occurs when attaching a template to an existing project in Adobe Workfront related to system-wide sharing permissions settings.|

For the latest product documentation, see the [Adobe [!DNL Workfront] documentation home page](https://experienceleague.adobe.com/en/docs/workfront/using/home).
-->

+++

## GenStudio for Performance Marketing {#genstudio-marketing}

尋找 [!DNL GenStudio for Performance Marketing] 的最新版本資訊和新文件。

+++發行說明和文件

請參閱 GenStudio for Performance Marketing [2025.12.12 - 發行說明](https://experienceleague.adobe.com/zh-hant/docs/genstudio-for-performance-marketing/user-guide/release-notes#latest)

<!-- ### New tutorials for GenStudio for Performance Marketing {#genstudio-tutorials}

Video tutorials for GenStudio for Performance Marketing.

|Published|Name|Type|Description |
| -----------| ---------- | ---------- | ---------- |
|October 2025|[Assign Brand permissions](https://experienceleague.adobe.com/en/docs/genstudio-for-performance-marketing/user-guide/intro/configure-brand-permissions)| Video |Learn about assigning entitlements for GenStudio for Performance Marketing Brand creators and editors.| -->

**GenStudio for Performance Marketing 資源**

* [Adobe GenStudio for Performance Marketing](https://experienceleague.adobe.com/zh-hant/browse/genstudio-for-performance-marketing) (全自助功能)
* GenStudio for Performance Marketing 的[產品文件](https://experienceleague.adobe.com/zh-hant/docs/genstudio-for-performance-marketing/user-guide/home)
* [GenStudio for Performance Marketing](https://business.adobe.com/products/genstudio-for-performance-marketing.html) 產品資訊

+++

## [!DNL Mix Modeler] {#mix-modeler}

+++發行說明和文件

如需詳細資訊，請參閱Mix Modeler [2025年11月 — 發行說明](https://experienceleague.adobe.com/zh-hant/docs/mix-modeler/using/releases/latest)

+++

## Adobe [!DNL Advertising] {#advertising}

尋找 [!DNL Adobe Advertising] 的最新版本資訊和新文件。

+++發行說明

若要瀏覽 Adobe [!DNL Advertising] 說明，請造訪 [Adobe Advertising 文件](https://experienceleague.adobe.com/zh-hant/docs/advertising)。

### [!DNL Advertising DSP] 中的新功能 {#advertising-dsp}

2025年12月17日

查看 [&#x200B; [!DNL Advertising DSP]中的新增功能](https://experienceleague.adobe.com/zh-hant/docs/advertising/dsp/home)

### [!DNL Advertising Search, Social, & Commerce] 中的新功能 {#advertising-search}

2025年11月12日

查看 [&#x200B; [!DNL Advertising Search, Social, & Commerce]中的新增功能](https://experienceleague.adobe.com/zh-hant/docs/advertising/search-social-commerce/home)

+++

## [!DNL Adobe Pass] {#pass}

[!DNL Adobe Pass] 是多螢幕電視平台，可協助電視台、有線電視網路和服務提供者，讓他們打造具吸引力的個人化觀看體驗並藉此創造營收。

+++文件

請參閱 [Adobe Pass 文件](https://experienceleague.adobe.com/zh-hant/docs/pass)，找到特定版本的資訊、系統需求、限制、已修正的問題和已知問題。

+++

## [!DNL Document Cloud] {#doc-cloud}

檢視針對[!DNL Document Cloud]發佈的新教學課程，包括[!DNL Acrobat Services]和[!DNL Acrobat Sign]。

+++最新教學課程和其他課程

| 發佈日期 | 應用程式 | 名稱 | 類型 | 說明 |
| -----------| ---------- | ---------- | ---------- |---------- |
| 2026 年 1 月 | Acrobat | [PDF協助工具要點](https://experienceleague.adobe.com/en/courses/acr-pdf-accessibility-essentials) | 課程 | 瞭解如何使用Acrobat Pro、Premium和Studio建立、檢查和修正PDF以提供協助工具。 |
| 2026 年 1 月 | Acrobat | [複雜表格](https://experienceleague.adobe.com/en/docs/document-cloud-learn/acrobat-learning/accessibility-tutorials/complex-tables) | 影片 | 瞭解如何在Acrobat中為協助工具標籤表格。 |
| 2026 年 1 月 | Acrobat | [標籤的基本概念](https://experienceleague.adobe.com/en/docs/document-cloud-learn/acrobat-learning/accessibility-tutorials/basics-tagging) | 影片 | 瞭解如何標籤PDF檔案以方便存取的基礎知識。 |
| 2026 年 1 月 | Acrobat | [在Acrobat中建立可存取的PDF](https://experienceleague.adobe.com/en/docs/document-cloud-learn/acrobat-learning/accessibility-tutorials/accessible-acrobat) | 影片 | 瞭解如何從Acrobat開始建立無障礙的PDF。 |
| 2026 年 1 月 | Acrobat | [[!UICONTROL 設定為可存取]引導式動作](https://experienceleague.adobe.com/en/docs/document-cloud-learn/acrobat-learning/accessibility-tutorials/make-accessible-action) | 影片 | 瞭解如何使用&#x200B;_[!UICONTROL 讓可存取]_&#x200B;引導式動作，讓您的PDF檔案可供存取。 |
| 2026 年 1 月 | Acrobat Sign | [建立適合行動裝置的檢視](https://experienceleague.adobe.com/en/docs/document-cloud-learn/sign-learning-hub/mobile/mobile-tutorials/create-mobile-friendly) | 影片 | 瞭解如何在沒有任何開發人員支援的情況下，順暢地產生適合行動裝置的檔案。 |
| 2026 年 1 月 | Acrobat Sign | [適合行動裝置的檢視](https://experienceleague.adobe.com/en/docs/document-cloud-learn/sign-learning-hub/mobile/mobile-tutorials/mobile-friendly) | 影片 | 瞭解如何使用適合行動裝置的檢視在您的行動裝置上完成表單。 |
| 2026 年 1 月 | Acrobat Sign | [建置自訂工作流程](https://experienceleague.adobe.com/en/docs/document-cloud-learn/sign-learning-hub/admin-set-up/getting-started-admin/building-a-custom-workflow) | 影片 | 瞭解如何建立和使用自訂工作流程，以加速建立和傳送協定的程式。 |

若要瀏覽 [!DNL Document Cloud] 教學課程，請參閱：

* [Adobe Acrobat](https://experienceleague.adobe.com/zh-hant/docs/document-cloud-learn/acrobat-learning/overview)
* [Adobe Acrobat Sign](https://experienceleague.adobe.com/zh-hant/docs/document-cloud-learn/sign-learning-hub/overview)
* [Adobe Acrobat Services API 教學課程](https://experienceleague.adobe.com/zh-hant/docs/acrobat-services-learn/tutorials/overview)
* [Document Cloud 學習與支援](https://helpx.adobe.com/tw/support/document-cloud.html)

+++

## 適用於企業的 [!DNL Creative Cloud] {#creative-cloud}

檢視針對[!DNL Adobe Creative Cloud for enterprise]中的應用程式(包括Firefly)發佈的新教學課程。

+++最新教學課程

<!-- |Published|Application|Name|Type|Description |
| -----------| -----------|---------- | ---------- | ---------- |
| September 2025 | Firefly | [Create a Custom Model](https://experienceleague.adobe.com/en/docs/creative-cloud-enterprise-learn/cce-learning-hub/fireflyoverview/firefly-tutorials/custom-model) |  Video | Learn how to build Custom Models in Firefly to create fresh new imagery for your organization's brand using specific types of styles--such as backgrounds, colors, and hues. |
| September 2025 | Firefly | [Background removal and replacement](https://experienceleague.adobe.com/en/docs/creative-cloud-enterprise-learn/cce-learning-hub/fireflyoverview/firefly-creative-production/background) |  Video | Learn how to use Firefly Creative Production to remove and/or replace backgrounds at scale. | -->

瀏覽我們的[適用於企業的 Creative Cloud](https://experienceleague.adobe.com/zh-hant/docs/creative-cloud-enterprise-learn/cce-learning-hub/overview) 教學課程資料庫，以了解更多資訊。

+++

## 客戶資料管理 - 對話 {#voices}

身為客戶資料管理技術和行銷實務主管和專家的您，[客戶資料管理對話](https://experienceleague.adobe.com/zh-hant/docs/events/customer-data-management-voices-recordings/overview)是您的最終目的地。此教學課程集合是您聆聽同行的意見、獲取靈感並了解 MarTech 中發展的一站式中心。不用報名，按一下即可觀看。

## 數位體驗藍圖 {#blueprints}

[數位體驗藍圖](https://experienceleague.adobe.com/zh-hant/docs/blueprints-learn/architecture/overview)是可重複的實作，可讓您處理策略，並快速解決既定的業務問題。每個藍圖都提供一系列成品，解釋高價值業務問題、架構、實作步驟、技術考量以及相關文件的連結。

<!-- ## ![Icon](/assets/certification-badge.png) Certification{#certification}

Attention all Adobe certification candidates! Visit the Experience Cloud [Certification](https://experienceleague.adobe.com/en/docs/certification/program/overview) site on Experience League. 

+++Details

The [Experience Cloud Certification](https://experienceleague.adobe.com/en/docs/certification/program/overview) site is your one-stop shop for all [!DNL Experience Cloud] certification-related content and is updated regularly with:

* Available certifications
* Certification renewals for Adobe applications
* Certification program updates

And more! Head over to [Adobe Certification](https://experienceleague.adobe.com/en/docs/certification/program/overview) on Experience League and start your certification journey today!

+++ -->
